...
const LeadList = () => {
  const [leadSearch, { data }] = useLeadSearchMutation();
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        ...
        const searchParams = {
          search: debouncedSearch.trim() || null, // Only send search if it has content
          client_id: 1,
          lead_id: null,
          name: null,
          mobile: null,
        ...
        const res = await leadSearch(searchParams).unwrap();
        setLeads(res?.data || []);
      } catch (err) {
